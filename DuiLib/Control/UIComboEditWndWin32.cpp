...
CComboEditWndWin32::CComboEditWndWin32() : m_pOwner(NULL), m_hBkBrush(NULL), m_bInit(false), m_bDrawCaret(false)
{
}

void CComboEditWndWin32::Init(CComboExUI* pOwner)
{
	m_pOwner = pOwner;
	RECT rcPos = CalPos();
	UINT uStyle = 0;
...
RECT CComboEditWndWin32::CalPos()
{
...
LPCTSTR CComboEditWndWin32::GetWindowClassName() const
{
	return _T("ComboEditWnd");
}

LPCTSTR CComboEditWndWin32::GetSuperClassName() const
{
	return WC_EDIT;
}

void CComboEditWndWin32::OnFinalMessage(HWND hWnd)
{
	m_pOwner->Invalidate();
...
LRESULT CComboEditWndWin32::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	LRESULT lRes = 0;
	BOOL bHandled = TRUE;
	if( uMsg == WM_CREATE ) {
...
LRESULT CComboEditWndWin32::OnKillFocus(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT CComboEditWndWin32::OnEditChanged(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
	if( !m_bInit ) return 0;
...
void CComboEditWndWin32::edit_SetPasswordMode(BOOL bPasswordMode)
{
...
void CComboEditWndWin32::edit_SetSel(int ichStart, int ichEnd)
{
	if(ichStart < 0 || ichEnd < 0)
	{
...
void CComboEditWndWin32::edit_SetText(LPCTSTR pstrText)
{
	Edit_SetText(m_hWnd, pstrText);
}

void CComboEditWndWin32::edit_LimitText(UINT uMax)
{
	Edit_LimitText(m_hWnd, uMax);
}

void CComboEditWndWin32::edit_SetReadOnly(BOOL bReadOnly)
{
	Edit_SetReadOnly(m_hWnd, bReadOnly);
}

void CComboEditWndWin32::edit_SetPasswordChar(TCHAR cPasswordChar)
{
	Edit_SetPasswordChar(m_hWnd, cPasswordChar);
}

void CComboEditWndWin32::edit_ReplaceSel(LPCTSTR lpszReplace)
{
