...
bool CShadowWin32UI::Initialize(HINSTANCE hInstance)
{
	if (s_bHasInit)
		return false;
...
void CShadowWin32UI::Create(CPaintManagerUI* pPaintManager)
{
	if(!m_bIsShowShadow)
		return;
...
void CShadowWin32UI::ShowShadow(bool bShow)
{
	m_bIsShowShadow = bShow;
}

bool CShadowWin32UI::IsShowShadow() const
{
	return m_bIsShowShadow;
}


void CShadowWin32UI::DisableShadow(bool bDisable) {
...
bool CShadowWin32UI::IsDisableShadow() const {

	return m_bIsDisableShadow;
}

bool CShadowWin32UI::SetSize(int NewSize)
{
...
bool CShadowWin32UI::SetSharpness(unsigned int NewSharpness)
{
	if(NewSharpness > 35)
		return false;
...
bool CShadowWin32UI::SetDarkness(unsigned int NewDarkness)
{
	if(NewDarkness > 255)
		return false;
...
bool CShadowWin32UI::SetColor(COLORREF NewColor)
{
	m_Color = NewColor;
	if(m_hWnd != NULL && (SS_VISABLE & m_Status))
		Update(GetParent(m_hWnd));
...
bool CShadowWin32UI::SetImage(LPCTSTR szImage)
{
	if (szImage == NULL)
		return false;
...
bool CShadowWin32UI::CopyShadow(CShadowBaseUI* pShadow)
{
	if (m_bIsImageMode) {
