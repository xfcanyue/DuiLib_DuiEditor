...
namespace DuiLib
{
	...
	SIZE CVerticalLayoutUI::EstimateSize(SIZE szAvailable)
	{
		...
		if(IsAutoCalcWidth() || IsAutoCalcHeight())
		{
			...
			if(IsAutoCalcHeight())
			{
				...
				if(GetCount() > 1) sz.cy += (GetCount() - 1) * GetChildPadding(); //注意此处ChildPadding需要经过DPI计算。
			}
	...
	bool CVerticalLayoutUI::CalcPos(CControlUI *pChildControl, CDuiRect &rcChild) //子控件调用这个，计算子空间的rect
	{
		return CContainerUI::CalcPos(pChildControl, rcChild);
