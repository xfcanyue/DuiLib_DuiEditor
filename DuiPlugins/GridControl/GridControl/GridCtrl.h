...
#define GRIDCONTROL_NO_TITLETIPS   // 没浮动提示内容Do not use titletips for cells with large data
...
typedef struct tagGV_CUSTOMDRAW {
	NMHDR   hdr;
	GV_ITEM item;
	CDC		*pDC;
	CRect	rc;
...
#define GVN_CUSTOMDRAW			CDRF_NOTIFYITEMDRAW

/////////////////////////////我添加的//////////////////////////////////
#include "CellType/GridCellCombo.h"
#include "CellType/GridCellCheck.h"
#include "CellType/GridCellDateTime.h"
//单元格类型
#define cellTextBox    RUNTIME_CLASS(CGridCell)			//0 //文本框   
#define cellComboBox   RUNTIME_CLASS(CGridCellCombo)	//1 //下拉框	
#define cellCheckBox   RUNTIME_CLASS(CGridCellCheck)	//2 //复选框	
#define cellDataTime   RUNTIME_CLASS(CGridCellDateTime) //3 //日期框
	
//////////////////////////////////////////////////////////////////////////

class CGridCtrl;

class XF_GRIDCONTROL_CTRL CGridTracker: public CRectTracker
{
public:
...
class XF_GRIDCONTROL_CTRL CGridCtrl : public CWnd
{
...
/* 我添加的内容 Added By Liqs99---------------------------------------------------------*/
	friend class CGridTracker;

	BOOL Create(int x, int y, int nWidth, int nHeight, CWnd* parent, UINT nID,
                DWORD dwStyle = WS_CHILD | WS_BORDER | WS_TABSTOP | WS_VISIBLE);
	
	//是否自动重画表格
	void SetAutoReDraw(BOOL bReDraw=TRUE);
	BOOL GetAutoDraw();
		
	//导出表格内容
	BOOL ExportToExcel(CString FileName, BOOL bShowFileDlg=TRUE);
	BOOL ExportToHtml(CString FileName, BOOL bShowFileDlg=TRUE);

	//是否只选中一行
	BOOL IsSelectedSingleRow();
	//返回选中行，多行，非ListMode无效
	BOOL GetSelectedRows(CUIntArray &arRow);
	//返回选中行，单行，非ListMode无效
	int GetSelectedRow();
	//是否选中行
	BOOL IsRowSelected(int row);
	
	//返回行的引用
	GRID_ROW &Row(int nRow);

	//返回单元格引用
	CGridCell &Cell(int nRow, int nCol);

	//返回默认单元格引用
	CGridDefaultCell &CellDef();
	CGridDefaultCell &CellFixedRowDef();
	CGridDefaultCell &CellFixedColDef();
	CGridDefaultCell &CellFixedRowColDef();
	
	//返回ComboBox引用类
	CGridCellCombo &ComboBox(int nRow, int nCol);

	//返回CheckBox引用类
	CGridCellCheck &CheckBox(int nRow, int nCol);

	//返回DateTime引用类
	CGridCellDateTime &DateTime(int nRow, int nCol);

	//初始化模式设定，表格的常用格式
	void FormatReport();

	//选中一行，使获得焦点，确保该行可见
	BOOL SelectRow(int row,int col=-1,BOOL bForceRedraw=TRUE, BOOL bSelectCells=TRUE);

	//设置整行的文字颜色
	void SetRowTextColor(int row, COLORREF clr);

	//选择所有
	BOOL SelectRowsAll();
	//反向选择
	BOOL SelectRowsInvert();

	//取消所有选中行
	void ClearSelectedRows();
	
	//设置是否允许高亮显示单元格背景
	void SetEnableHighLightBkColor(BOOL bEnable) { m_bHighLightBkColor = bEnable; }
	//获取是否允许高亮显示单元格背景
	BOOL GetEnableHighLightBkColor()			   { return m_bHighLightBkColor; }

	//设置是否允许高亮显示单元格文字
	void SetEnableHighLightTextColor(BOOL bEnable) { m_bHighLightTextColor = bEnable; }
	//获取是否允许高亮显示单元格文字
	BOOL GetEnableHighLightTextColor()			   { return m_bHighLightTextColor; }

	void SetHighLightBkColor(COLORREF clr)		{m_crHighLightBkColor = clr;}
	COLORREF GetHighLightBkColor()	{return m_crHighLightBkColor;}
	void SetHighLightTextClr(COLORREF clr)	{m_ctHighLightTextClr = clr;}
	COLORREF GetHighLightTextClr(){return m_ctHighLightTextClr;}

	void SetFrameFocusCellColor(COLORREF clr)	{m_crFrameFocusCellColor = clr;}
	COLORREF GetFrameFocusCellColor(){return m_crFrameFocusCellColor;}

	//设置打印表格中间标题
	void SetPrintCenterTitle(LPCTSTR str, BOOL bPrint);
	//设置打印表格右标题
	void SetPrintRightTitle(LPCTSTR str, BOOL bPrint);
	//设置打印表格左边第二副标题
	void SetPrintLeftSecondTile(LPCTSTR str, BOOL bPrint);

	void SetViewFixedColNo(BOOL bView=TRUE) {m_bViewFixedColNo = bView; }
	BOOL GetViewFixedColNo(){ return m_bViewFixedColNo; }
protected:
	BOOL m_bViewFixedColNo;
	BOOL m_bHighLightBkColor;
	BOOL m_bHighLightTextColor;
	
	CGridTracker m_RectTracker;
//增加统计行
//////////////////////////////////////////////////////////////////////////
public:
	void SetStatRow(int row, int nStat);
protected:
	int m_nStatRows; //默认的统计行有两行，必须是在最后两行。 第1行是空行，第2行是数据
//////////////////////////////////////////////////////////////////////////
protected:
	//高亮显示时的背景色和前景色
	COLORREF  m_crHighLightBkColor;
	COLORREF  m_ctHighLightTextClr;

	COLORREF m_crFrameFocusCellColor;

	CString m_strCenterTitle;
	BOOL m_bPrintCenterTitle;

	CString m_strRightTitle;
	BOOL m_bPrintRightTitle;

	CString m_strLeftSecondTile;
	BOOL m_bPrintLeftSecondTile;
...
public:
	...
	//同行/列中的固定单元格作为焦点单元格时是否高亮显示并且使用凹陷边缘。
...
public:
	...
	//保存为xml文件 modify by liqs99
	//bMemory = TRUE, file为文件名
	//bMemory = FALSE, file为文件内容
	BOOL SaveXml(CString &file, BOOL bMemory = FALSE);	

	//载入xml文件 modify by liqs99, bMemory=是否载入内存文件
	//bMemory = TRUE, file为文件名
	//bMemory = FALSE, file为文件内容
	BOOL LoadXml(LPCTSTR file, BOOL bMemory=FALSE);	
#endif
...
protected:
	...
	LRESULT SendCustomDrawToParent(GV_CUSTOMDRAW *pCustomDraw) const;
