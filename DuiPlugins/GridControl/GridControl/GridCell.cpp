...
void CGridCell::SetFixed(BOOL bFixed)
{ 
	if(bFixed)
	{
		m_nState |= GVIS_FIXED;//|GVIS_FIXEDROW; 
		SetBackClr(GetGrid()->GetFixedBkColor());
		SetTextClr(GetGrid()->GetFixedTextColor());
	}
	else
	{
		m_nState &= ~GVIS_FIXED;//|GVIS_FIXEDROW; 
		SetBackClr(GetGrid()->GetTextBkColor());
		SetTextClr(GetGrid()->GetTextColor());
	}
}
...
BOOL CGridCell::Edit(int nRow, int nCol, CRect rect, CPoint /* point */, UINT nID, UINT nChar)
{
	...
	if(m_bEditable)  //单元格是否可编辑, Added by Liqs99
	{   
...
//////////////////////////////////////////////////////////////////////////
//Added By Liqs99
COLORREF CGridCell::GetHighLightBkColor()			
{
	return m_pGrid->GetHighLightBkColor();
}

COLORREF CGridCell::GetHighLightTextClr()			
{
	return m_pGrid->GetHighLightTextClr();
}
...
CGridDefaultCell::CGridDefaultCell() 
{
    ...
    //ncm.cbSize = sizeof(NONCLIENTMETRICS); //低版本, XP以前的版本
	ncm.cbSize = sizeof (NONCLIENTMETRICS) - sizeof(ncm.iPaddedBorderWidth); //高版本
