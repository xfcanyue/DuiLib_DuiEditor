...
namespace DuiLib
{
	static CMapPtrToPtr g_MapTimerID_TO_CRollTextExUI;
	static void CALLBACK TimerProc(HWND hWnd,UINT nMsg,UINT idEvent,DWORD dwTime)  
	{
		CRollTextExUI *pObject;
		if (g_MapTimerID_TO_CRollTextExUI.Lookup((void *) idEvent, (void*&)pObject))
		{
	...
	void CRollTextExUI::BeginRoll()
	{
		if(m_idEventTimer == 0)
		{
			...
			g_MapTimerID_TO_CRollTextExUI.SetAt((void *) m_idEventTimer, this);
		}
	}
	...
	void CRollTextExUI::EndRoll()
	{
		if(m_idEventTimer != 0)
		{
			...
			g_MapTimerID_TO_CRollTextExUI.RemoveKey((void *) m_idEventTimer);
			m_idEventTimer = 0;
		}
	}
