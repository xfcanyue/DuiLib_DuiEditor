...
{
	m_pSci->sci_SetLexer(SCLEX_XML);		//设定词法解析器
	m_pSci->sci_SetCodePage(SC_CP_UTF8);	//编码

	m_pSci->SendEditor(SCI_STYLESETFONT, STYLE_DEFAULT, (LPARAM)"Courier New");//(LPARAM)"Courier New");//"微软雅黑");
	...
	//代码折叠
	m_pSci->sci_SetMarginTypeN(VIEW_MARGIN_FOLD, SC_MARGIN_SYMBOL);	//页边类型
	m_pSci->sci_SetMarginMaskN(VIEW_MARGIN_FOLD, SC_MASK_FOLDERS);	//页边掩码
	m_pSci->sci_SetMarginWidthN(VIEW_MARGIN_FOLD,20);				//页边宽度
	m_pSci->sci_SetMarginSensitiveN(VIEW_MARGIN_FOLD, TRUE);		//页边响应鼠标点击
	...
	m_pSci->sci_SetTabWidth(4);	//制表符宽度
	m_pSci->sci_SetIndentationGuides(SC_IV_LOOKBOTH);	//缩进提示线
	m_pSci->sci_SetIndent(4);	//缩进提示线的字符宽度
		...
		case SCN_MODIFIED:
			{
				if(pMsg->linesAdded!=0 &&
					m_pSci->sci_GetMarginWidthN(VIEW_MARGIN_LINENUMBER)!=0) //自动调整行号的宽度
				{
					int lineNumber = m_pSci->sci_GetLineCount();
