...
vector< pair<int, int> > XmlMatchedTagsHighlighter::getAttributesPos(int start, int end)
{
	...
	CStringA buf;
	_pEditView->sci_GetTextRange(start, end, buf);
...
XmlMatchedTagsHighlighter::XmlMatchedTagsPos XmlMatchedTagsHighlighter::tagMatch(bool doHiliteAttr) 
{
	...
	int docStart = 0;
	int docEnd = _pEditView->sci_GetLength();
	_pEditView->sci_SetIndicatorCurrent(SCE_UNIVERSAL_TAGMATCH);
	_pEditView->sci_IndicatorClearRange(docStart, docEnd);
