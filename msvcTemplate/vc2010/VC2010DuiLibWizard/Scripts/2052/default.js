...
function CreateGuids() {
    ...
    wizard.AddSymbol("APP_SINGLE_INSTANCE", strVal);
}
...
// 		group.Filter = strSrcFilter;
		...
		proj.Object.AddFilter('资源文件').Filter = 'ico;rc;bin;zip';
...
function GetTargetName(strName, strProjectName)
{
	try
	{
		...
		if (strName == 'DuiLibTemplate.h')
		    strTarget = strProjectName + '.h';

		if (strName == 'DuiLibTemplate.cpp')
		    strTarget = strProjectName + '.cpp';

		if (strName == 'DuiLibTemplate.ico')
		    strTarget = strProjectName + '.ico';

		if (strName == 'DuiLibTemplate.rc')
	...
	try
	{
		...
		while (!strTextStream.AtEndOfStream)
		{
			...
			if (strTpl != '')
			{
				...
				if (strExt == ".bmp" || strExt == ".jpg" || strExt==".png" || strExt == ".ico" || strExt == ".gif" || strExt == ".rtf" || strExt == ".css" || strExt == ".zip")
					bCopyOnly = true;
